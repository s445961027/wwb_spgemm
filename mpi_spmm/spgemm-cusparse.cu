...
const char* version_name = "cuSPARSE SpGEMM";
...
void preprocess(dist_matrix_t *matA, dist_matrix_t *matB) {
    ...
    cusparseCreateCsr(&p->matA, matA->global_m, matA->global_m, matA->global_nnz, matA->gpu_r_pos, matA->gpu_c_idx, matA->gpu_values,
        CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I, CUSPARSE_INDEX_BASE_ZERO, CUDA_R_32F);
    cusparseCreateCsr(&p->matB, matB->global_m, matB->global_m, matB->global_nnz, matB->gpu_r_pos, matB->gpu_c_idx, matB->gpu_values,
        CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I, CUSPARSE_INDEX_BASE_ZERO, CUDA_R_32F);
    cusparseCreateCsr(&p->matC, matA->global_m, matB->global_m,                0,            NULL,            NULL,             NULL,
        CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I, CUSPARSE_INDEX_BASE_ZERO, CUDA_R_32F);
...
void spgemm(dist_matrix_t *mat, dist_matrix_t *matB, dist_matrix_t *matC,int process_num,int pid) {
    const data_t alpha = 1.0, beta = 0.0;
    info_ptr_t p = (info_ptr_t)mat->additional_info;
    // delete the working space for multiple tests
    if (p->dBuffer1) {
        cudaFree(p->dBuffer1); p->dBuffer1 = NULL;
    }
    if (p->dBuffer2) {
        cudaFree(p->dBuffer2); p->dBuffer2 = NULL;
    }
    ...
    // CUSPARSE_SPGEMM_DEFAULT: Invokes cusparseSpGEMM_compute twice.
    // The first invocation provides an upper bound of the memory required for the computation.
    ...
    // returns the sizes of the sparse matrix "p->matC".
    ...
    // sets the pointers of the sparse matrix descriptor "p->matC".
    CHECK_CUSPARSE( cusparseCsrSetPointers(p->matC, matC->gpu_r_pos, matC->gpu_c_idx, matC->gpu_values) )
    CHECK_CUSPARSE( cusparseSpGEMM_copy   (p->handle, CUSPARSE_OPERATION_NON_TRANSPOSE, 
        CUSPARSE_OPERATION_NON_TRANSPOSE, &alpha, p->matA, p->matB, &beta, 
