...
int main(int argc, char** argv) {
    ...
    dim3 Block(32,32,1);
    dim3 Grid(N/Block.x/THREAD_SIZE_N,M/Block.y/THREAD_SIZE_M,1);
    // for (int run = 0 ; run < nIter; run ++ ) {
    //     MatrixMul_naive <<< Grid, Block >>>(d_A, d_B, d_C, M, K, N);

    // }
    for (int run = 0 ; run < nIter; run ++ ) {
        MatrixMul2<32,8,32,4,4> <<< Grid, Block >>>(d_A, d_B, d_C, M, K, N);
